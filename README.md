...
Пиковый RPS на бэкенд = 6,6 * 10^6 * 4 / (2 * 60 * 60) = 3700 RPS (по данным Netflix - 20000RPS в пике)

RPS на статику = 160 * 3.85 * 33 * 10^6 / 24 * 60 * 60 = 266560 RPS
...
```
Размер записи в табличке:
users = 8 + 64 + 64 + 128 + 64 + 8 = 336 байт
subscriptions = 8 + 8 + 8 + 1 + 1 = 26
favourites = 8 + 8 + 8 = 24
rates = 8 + 8 + 8 + 2 = 26

sessions = 8 + 64 + 8 + 8 = 88

viewed = 8 + 8 + 8 + 8 + 8 = 40

content = 8 + 128 + 128 + 1024 + 512 + 8 + 4 + 128 + 8 + 2 + 4 ~ 2000
movies = 4 + 128 + 128 + 8 = 268
tv_shows = 4 + 4 + 8 = 16
...
Будем считать, что таблица с контентом уместится в 2ГБ.
...
Внутри кластера будем хранить контент в репликах, количество реплик пропорционально популярности. 
...
Количество CDN-серверов: 55976 / 36 ~ 1600 серверов
| Название       | CPU | RAM | SSD | Количество | 
| ------------- |:-------------:|:-------------:|:-------------:|:-------------:|
| CDN     | 32 | 512 ГБ | 360 ТБ |  1600 + 800|
| Фронтенд    | 32 | 256 ГБ| 128 ГБ | 8 + 4 | 
| Бэкенд | 32 | 256 ГБ | 512 ГБ | 8 + 4  |
| Nginx | 32 | 256 ГБ | 256 ГБ | 8 + 4|
| Авторизация | 32 | 128 ГБ | 256 ГБ | 8 + 4 |
| Рекомендации | 32 | 256 ГБ | 256 ГБ | 4 + 2 | 
| Каталог |  8 | 512 ГБ | 256 ГБ| 4 + 2 |
| Счетчик просмотров | 8 | 128 ГБ | 256 ГБ| 4 + 2 |
